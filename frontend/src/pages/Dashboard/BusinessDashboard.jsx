...
const BusinessDashboard = () => {
  ...
  return (
    <div style={{ maxWidth: '1440px', margin: '0 auto', paddingBottom: '60px' }}>
      ...
      {/* 🏙️ BRANDING HEADER (Exact from Reference) */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '45px' }}>
        ...
        <h1 style={{ fontSize: '32px', fontWeight: '900', color: '#0f172a', letterSpacing: '-1.8px', margin: 0 }}>VisionBridge</h1>
      </div>

      <h2 style={{ fontSize: '42px', fontWeight: '950', color: '#0f172a', marginBottom: '35px', letterSpacing: '-2.5px' }}>Dashboard</h2>

      {/* 🔔 SIP END ALERT (Preserved Feature) */}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '35px' }}>
        <div className="card">
          ...
          {data.topFunds?.map((fund, idx) => (
            <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '20px 24px', background: 'rgba(255, 255, 255, 0.05)', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: '16px', marginBottom: '12px' }}>
              <span style={{ fontWeight: '800', color: '#cbd5e1' }}>0{idx + 1} &nbsp; {fund.scheme_name}</span>
              <span style={{ fontWeight: '900', color: '#34d399' }}>₹{formatINR(fund.invested_value)}</span>
            </div>
          ))}
        </div>
