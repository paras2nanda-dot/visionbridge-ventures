    ...
    <>
      <style>{`
        ...
        @media (min-width: 769px) {
          ...
          .settings-sheet { border-radius: 20px; border: 2.5px solid var(--border); }
          .drag-indicator { display: none; }
        }

        .theme-card {
          background: var(--bg-main) !important; border: 2.5px solid var(--border) !important;
      ...
      <div className="settings-backdrop" onClick={onClose}>
        <div className="settings-sheet" onClick={e => e.stopPropagation()}>
          ...
          <div style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button 
              onClick={onClose} 
              style={{ background: 'var(--bg-main)', border: '2px solid var(--border)', color: 'var(--text-main)', width: '32px', height: '32px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 'bold' }}
            >
              ✕
            </button>
          </div>
          ...
          <div style={{ padding: '0 24px 32px 24px' }}>
            <h3 style={{ margin: '0 0 8px 0', color: 'var(--text-main)', fontSize: '16px', fontWeight: '800' }}>Platform Theme</h3>
            <p style={{ margin: '0 0 24px 0', color: 'var(--text-muted)', fontSize: '13px', fontWeight: '600', lineHeight: 1.4 }}>Choose your visual style. Changes apply across the dashboard instantly.</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {themeOptions.map((opt) => (
                ...
                >
                  ...
                  <div style={{ fontWeight: '900', color: theme === opt.id ? '#0284c7' : 'var(--text-main)', fontSize: '14px' }}>
                    {opt.label}
                  </div>
                </button>
              ))}
