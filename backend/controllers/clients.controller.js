...
export const updateClient = async (req, res) => {
  ...
  try {
    ...
    fields.forEach(f => {
        if (c[f.key] && String(c[f.key]) !== String(old[f.key])) {
            changes.push(`• ${f.label}: ${old[f.key] || 'Empty'} → ${c[f.key]}`);
        }
    });
...
export const deleteClient = async (req, res) => {
  ...
  try {
    ...
    await logActivity(username, 'DELETE', full_name, `🗑️ Permanently removed client: ${full_name} (${client_code})`);
...
export const bulkDeleteClients = async (req, res) => {
  ...
  try {
    // 💡 TYPE-PROOF CASTING: Ensures array elements are strings for PostgreSQL ANY clause
